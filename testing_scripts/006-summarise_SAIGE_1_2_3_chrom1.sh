...
# Concat all of the results from chromosome 1 within each nPC
for n_geno_pcs in 5 10 15 20; do
        for f in ${catdir}/*npc${n_geno_pcs}_cis.txt
        do
                tail -n +2 $f >> ${catdir}/chr1_nPC_${n_geno_pcs}.txt
        done
...
# Execute R script to make summary plots and decide which nPC value to use
# scvi_gpu3 env
