...
# If submitting as an array job:
gene_list=${catdir}/test_genes.txt
gene=$(head $gene_list -n ${LSB_JOBINDEX} | tail -n 1)
# bsub -o logs/saige_array_test-%J-%I-output.log -e logs/saige_array_test-%J-%I-error.log -q normal -G team152 -n 4 -M 10000 -a "memlimit=True" -R "select[mem>10000] rusage[mem=10000] span[hosts=1]" -J "saige_array_test[1-9447]%100" < testing_scripts/run_SAIGE_1_2.sh 
...
if [ "$cis_only" = true ]; then
        ...
        tail -n +2 ${step2prefix}.txt >> ${catdir}/chr${gene_chr}_nPC_${n_geno_pcs}.txt
